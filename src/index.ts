...
import { convertRoot, type Spec, type SpecCommand, type SpecFlag, type SpecArg } from "./convert.js";
import { renderKDL } from "./kdl.js";

export { convertRoot } from "./convert.js";
export { renderKDL, kdlQuote, kdlQuoteAlways, needsQuoting } from "./kdl.js";
export type {
  Spec,
  SpecArg,
  SpecCommand,
...
export function generate(cmd: Command, binName?: string): string {
  ...
  return `// @generated by commander-usage from Commander.js metadata\n${renderKDL(spec)}`;
}
...
export function generateJSON(cmd: Command, binName?: string): string {
  const spec = convertRoot(cmd);
  if (binName) spec.bin = binName;
  return JSON.stringify(toJSON(spec), null, 2) + "\n";
}
...
function toJSON(spec: Spec): Record<string, unknown> {

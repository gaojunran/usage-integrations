...
private val BUILTIN_FLAG_NAMES = setOf("help")
...
private fun convertFlag(opt: Option, context: Context?): SpecFlag {
    ...
    val isNegated = negatedName != null
    val positiveName = if (isNegated) negatedName!!.replace("--no-", "--") else ""
    ...
    var flag = SpecFlag(
        ...
        negate = if (isNegated && positiveName.isNotEmpty()) positiveName else "",
        deprecated = "",
...
private fun convertCommand(cmd: CoreCliktCommand): SpecCommand {
    ...
    return SpecCommand(
        ...
        help = context?.let { cmd.commandHelp(it) } ?: "",
        helpLong = "",
...
fun convertRoot(cmd: CliktCommand, binName: String? = null): Spec {
    ...
    return Spec(
        ...
        bin = name,
        version = "",
        about = context?.let { cmd.commandHelp(it) } ?: "",
        long = "",
        usage = "",
        flags = flags,
        args = args,

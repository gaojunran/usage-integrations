...
private fun convertArg(pd: ParameterDescription): SpecArg {
    ...
    val choices = extractChoices(pd)
    val defaultValue = pd.default?.toString()
    ...
    return SpecArg(
        ...
        `var` = variadic,
        hide = wp.hidden(),
        default = if (defaultValue != null && defaultValue.isNotEmpty()) listOf(defaultValue) else emptyList(),
        choices = choices,
    )
}

private fun extractChoices(pd: ParameterDescription): SpecChoices? {
    val type = pd.parameterized.type
    if (!Enum::class.java.isAssignableFrom(type)) return null

    @Suppress("UNCHECKED_CAST")
    val enumClass = type as Class<out Enum<*>>
    val values = enumClass.enumConstants.map { it.name }
    return SpecChoices(values)
}
...
private fun getRawCommands(jc: JCommander): List<Triple<String, JCommander, List<String>>> {
    return try {
        val field = jc.javaClass.getDeclaredField("commands")
        field.isAccessible = true
        @Suppress("UNCHECKED_CAST")
        val map = field.get(jc) as java.util.Map<Any, JCommander>
        val result = mutableListOf<Triple<String, JCommander, List<String>>>()
        for (entry in map.entrySet()) {
            val progName = entry.key
            val subJc = entry.value
            val progNameClass = (progName as Any)::class.java
            val name = progNameClass.getMethod("getName").invoke(progName) as String
            val aliasesField = progNameClass.getDeclaredField("aliases")
            aliasesField.isAccessible = true
            @Suppress("UNCHECKED_CAST")
            val aliases = aliasesField.get(progName) as? List<String> ?: emptyList()
            result.add(Triple(name, subJc, aliases))
        }
        result
    } catch (_: Throwable) {
        emptyList()
...
private fun convertCommand(name: String, jc: JCommander, aliases: List<String> = emptyList()): SpecCommand {
    ...
    for ((cmdName, subJc, cmdAliases) in getRawCommands(jc)) {
        cmds.add(convertCommand(cmdName, subJc, cmdAliases))
    }
...
fun convertRoot(jc: JCommander, binName: String? = null): Spec {
    ...
    val name = binName ?: jc.programName ?: paramsAnnotation?.commandNames?.firstOrNull() ?: ""
